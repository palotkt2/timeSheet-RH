...
const badAssignments = db.prepare(
...
const assignmentsToFix = db.prepare(
  `SELECT id, days FROM shift_assignments 
   WHERE days LIKE '%6%' 
   AND shift_name NOT LIKE '%Guardias%' 
   AND shift_name NOT LIKE '%FIN DE SEMANA%'`
).all();
const updateAssignment = db.prepare('UPDATE shift_assignments SET days = ? WHERE id = ?');
let assignFixed = 0;
