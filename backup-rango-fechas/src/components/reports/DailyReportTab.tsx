...
export default function DailyReportTab() {
  ...
  const getPhotoUrl = (empNum: string, date: string, type: 'entry' | 'exit') =>
    `/api/employee-photos/${empNum}?date=${date}&type=${type}`;
  ...
  const generateReport = async () => {
    ...
    try {
      ...
      let url: string;
      if (isRange) {
        url = `/api/multi-plant/reports/daily?startDate=${startDate}&endDate=${endDate}`;
      } else {
        url = `/api/multi-plant/reports/daily?date=${startDate}`;
      }
  ...
  const exportToCSV = () => {
    ...
    csvRows.push(
      'Fecha,Empleado,Nombre,Sesión,Entrada,Salida,Duración (hrs),Planta(s)',
    );
    ...
    link.setAttribute(
      'download',
      `reporte-diario-multiplanta-${reportData.startDate || reportData.date}${reportData.endDate && reportData.endDate !== reportData.startDate ? '_a_' + reportData.endDate : ''}.csv`,
    );
            ...
            <Table stickyHeader size="small">
              ...
              <TableBody>
                {reportData.employees.map((emp: DailyEmployee) => {
                  const rowDate = emp.date || reportData.date;
                  return (
                    <TableRow key={`${emp.employeeNumber}-${rowDate}`} hover>
                      {/* Fecha */}
                      <TableCell>
                        <Typography variant="body2">
                          {new Date(rowDate + 'T12:00:00').toLocaleDateString(
                            'es-MX',
                            {
                              weekday: 'short',
                              day: '2-digit',
                      ...
                      <TableCell sx={{ textAlign: 'center' }}>
                        {emp.lastExit
                          ? formatLocalTimeShort(emp.lastExit)
                          : '-'}
                      </TableCell>
                      <TableCell sx={{ textAlign: 'center' }}>
